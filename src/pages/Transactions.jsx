...
export default function Transactions() {
  ...
  const [tempStartMonth, setTempStartMonth] = useState(now.getMonth() + 1);
  ...
  const fetchTransactions = () => {
    ...
    if (queryObj.startMonth && queryObj.endMonth && queryObj.year) {
      ...
      const toStr = `${queryObj.year}-${String(queryObj.endMonth).padStart(2, '0')}-${String(toDate.getDate()).padStart(2, '0')}`;
      q.push(`from=${fromStr}&to=${toStr}`);
    }
